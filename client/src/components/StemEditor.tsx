...
export function StemEditor({ asset, expanded, onExpand, onCollapse }: StemEditorProps) {
  ...
  const handleAiGenerate = useCallback((stemName: string) => {
    setAiGenerating(stemName);
    // Simulate AI generation delay
    setTimeout(() => setAiGenerating(null), 2500);
  }, []);
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-canvas-accent">
          ...
          <div className="flex items-center gap-2">
            {/* AI Generate buttons */}
            <button
              className="px-2.5 py-1 text-[10px] font-semibold rounded-md bg-purple-900/30 text-purple-300 border border-purple-500/30 hover:bg-purple-500/20 transition-colors flex items-center gap-1"
            ...
            <button
              className="px-2.5 py-1 text-[10px] font-semibold rounded-md bg-purple-900/30 text-purple-300 border border-purple-500/30 hover:bg-purple-500/20 transition-colors flex items-center gap-1"
              onClick={() => handleAiGenerate("endtag")}
            >
              <svg width="10" height="10" viewBox="0 0 16 16" fill="currentColor"><path d="M8 1l2 5h5l-4 3 1.5 5L8 11l-4.5 3L5 9 1 6h5z"/></svg>
              Generate Endtag
            </button>
        ...
        <div className="px-4 pb-4 flex flex-col gap-1.5">
          {stems.map((stem, i) => {
            ...
            const isGenerating = aiGenerating === stem.name;
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <button
                    onClick={() => handleAiGenerate(stem.name)}
                    disabled={isGenerating}
                    className={`px-2 py-0.5 text-[9px] font-semibold rounded border flex items-center gap-1 transition-all ${
                      isGenerating
                        ? "bg-purple-500/20 text-purple-300 border-purple-500/40 animate-pulse"
                        : "bg-purple-900/20 text-purple-400/70 border-purple-500/20 hover:bg-purple-500/20 hover:text-purple-300 hover:border-purple-500/40"
                    }`}
                  >
                    <svg width="8" height="8" viewBox="0 0 16 16" fill="currentColor">
                      <path d="M8 1l2 5h5l-4 3 1.5 5L8 11l-4.5 3L5 9 1 6h5z"/>
                    </svg>
                    {isGenerating ? "Generating…" : "AI Variation"}
                  </button>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-canvas-accent bg-canvas-bg/30">
          <div className="flex items-center gap-2">
            <button className="px-3 py-1.5 text-[10px] font-semibold rounded-md bg-canvas-accent text-canvas-muted hover:text-canvas-text transition-colors">
              Reset All
            </button>
            <button className="px-3 py-1.5 text-[10px] font-semibold rounded-md bg-canvas-accent text-canvas-muted hover:text-canvas-text transition-colors">
              Export Stems
          ...
          <div className="flex items-center gap-2">
            <button className="px-3 py-1.5 text-[10px] font-semibold rounded-md bg-green-900/30 text-green-400 border border-green-500/30 hover:bg-green-500/20 transition-colors flex items-center gap-1">
              <svg width="8" height="8" viewBox="0 0 10 10" fill="currentColor"><polygon points="1,0 10,5 1,10" /></svg>
              Preview Mix
            </button>
            <button className="px-3 py-1.5 text-[10px] font-semibold rounded-md bg-canvas-highlight/20 text-canvas-highlight border border-canvas-highlight/30 hover:bg-canvas-highlight/30 transition-colors">
              Apply Changes
            </button>
          </div>
        </div>
